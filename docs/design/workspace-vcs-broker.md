...
Ensure-and-deploy is serialized by a per-profile host `flock`. Acquisition is bounded at 12 seconds. The lock covers state inspection, repair or restart requests, and an unhealthy replacement's bounded startup; it never covers a graceful drain. A caller that finds an intentional drain receives a retryable warning instead of waiting behind it. Starting a new service has a 10-second readiness bound. Broker guest-control SSH calls are independently bounded at 12 seconds. The stale-service replacement path can therefore spend up to 47.8 seconds in broker coordination: 12 seconds resolving the guest home, 12 seconds acquiring the lock, 1.8 seconds on three host probes, 12 seconds on owner-checked guest cleanup, and 10 seconds starting the replacement. Workspace discovery and hypervisor operations outside this broker coordination have their own bounds. A workspace-mode change away from the broker requests daemon-owned retirement and returns without waiting for its drain.

Health has two separate failure domains. Daemon health is an authenticated request directly to its host loopback port using the recorded token. Cloister makes three attempts, each bounded at 500 ms, with 100 ms and 200 ms backoffs; all three must fail before the daemon is replaced, for a maximum 1.8-second host-health delay. An authenticated `503` with `Retry-After` identifies an intentional drain and is not treated as a dead daemon. Tunnel health separately verifies the exact recorded SSH process and then sends an authenticated probe through the fixed guest endpoint. A healthy daemon with a failed tunnel path receives an immediate in-process tunnel-repair request; its PID and token stay unchanged, and command admission is never paused. Repair replaces only the exact tunnel claim, atomically refreshes the guest shims and owner configuration with the existing token, probes the guest path, and publishes the new tunnel PID. Host commands and post-command barriers already in flight continue when the tunnel is swapped. A client whose response was using the broken or replaced tunnel can receive a connection failure; the shim does not retry that ambiguous case because the command was admitted and may have mutated its repository.

The workspace hash covers the guest home, workspace configuration, and derived project session specifications. On a hash mismatch, ensure writes the desired configuration and asks the running daemon to apply it; on a build-identity mismatch it asks for replacement by the current Cloister build. The request returns immediately while the old broker continues serving its current project set. The daemon then closes admission to new commands, returns authenticated `503` plus `Retry-After` before any rejected command executes, and drains the set it already admitted. With no new admissions, steady traffic cannot starve the transition. A same-build configuration change rebuilds and swaps the mapper in process and refreshes upgraded guest shims without changing the token. A build change starts the current executable as the replacement and transfers tunnel/config ownership after the old command set drains. Failed or timed-out reload or replacement reopens the old service. The admission-closed window is bounded at ten minutes for the longest admitted command, plus at most 12 seconds for a same-build guest-shim refresh; build replacement startup is bounded at ten seconds. Keeping the fixed guest listener continuously open during a process replacement would require descriptor passing or a separate permanent front proxy; that complexity is disproportionate to a rare upgrade and would itself become a second credential-bearing lifecycle component. The build identity is the embedded Cloister version (or Go VCS build metadata for an unversioned development binary), not a hash of the executable.

Shutdown first stops admitting commands while leaving the listener and reverse tunnel available. New authenticated requests receive `503`, `Retry-After`, and a message telling the caller to retry. Already admitted commands run through the host operation and its post-command Mutagen barrier. Command output is written to a private spool and concurrently streamed, so a disconnected or half-open reader cannot block command execution or retain an active-command slot; each stalled network write is bounded at 15 seconds. The tunnel remains until healthy clients receive the real body and exit-code trailer. Only after the accepted set and response handlers finish does the daemon atomically remove its owned guest configuration, stop its exact tunnel, and exit. The drain bound is ten minutes, chosen to cover realistic authenticated pushes and hooks without allowing VM lifecycle operations to wait forever; the lifecycle controller allows five additional seconds for cleanup. `cloister stop` and other destructive lifecycle operations wait and print the active argv, project, and elapsed time every five seconds. If the drain expires, Cloister records and reports every interrupted command and guest project path, kills the daemon's process group, warns, and continues stopping or destroying the VM. Managed hard-stop paths also target the process group. Each host command also has a sibling watchdog connected to the daemon by a private pipe; if even a raw `SIGKILL` removes only the daemon PID, pipe closure makes the watchdog kill the remaining private process group before the child can continue independently. A `SIGSTOP` leaves the pipe open and deliberately allows an accepted child to continue: finishing a repository operation is safer than asynchronously killing it midway through mutation.
...
The guest shim transparently retries only an authenticated admission-time `503`, honoring a numeric `Retry-After` and otherwise backing off from one second to a five-second cap. Its total retry budget is 10 minutes 15 seconds, covering the maximum ten-minute drain, ten-second replacement startup, and five seconds of scheduling margin. Once the server admits a request it sends HTTP 200 and never converts execution or barrier failure into 503, so an ambiguous connection loss or ordinary command failure is never retried.
