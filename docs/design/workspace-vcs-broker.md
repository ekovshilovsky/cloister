...
guest git shim
  ...
  -> repo-scoped gh with explicit -R/--repo/GH_REPO: use a mapped project when available, otherwise run from host home
...
Commands tied to a mapped project flush before execution. A successful flush is a bidirectional convergence barrier, which makes guest edits visible in the host working tree and also imports completed host edits. Commands classified as working-tree-mutating flush again after the host process exits, even when it exits nonzero because a failed merge, rebase, or checkout can leave changes. The post-flush result takes precedence over a zero command exit because the guest view would otherwise be stale. Per-project locking serializes brokered VCS operations. Account-scoped `gh` commands and explicitly targeted repository commands issued outside a project have no project tree to synchronize, so they run from host home without a lock or barrier. Uncoordinated native host editors cannot be locked; concurrent edits are reconciled by the barriers and fail closed on a Mutagen conflict.
...
| `gh` and PRs | The `gh` shim runs allowed host commands with host authentication. Search, status, `auth status`, `repo list`, and read-only API endpoints without repository placeholders or a `repos/...` path are account-scoped and need no mapped project. PR, issue, run, workflow, release, `repo view`, and repository API endpoints use a mapped project unless `-R`/`--repo` or `GH_REPO` explicitly selects one. `gh api` remains restricted to read-only (GET, no request-body fields) because it runs with host credentials and an unrestricted write would allow account-level mutation (SSH keys, repo deletion) from the sandbox; run write API calls on the host. Aliases, extensions, authentication mutation, and repo clone or creation are rejected. Commands that invoke an editor or browser require explicit noninteractive flags. |
| Outside a mapped project | At the workspace routing root, account-scoped `gh` and explicitly targeted repository commands run from host home. Bare Git is refused with guidance to use `git -C`; implicitly targeted repository `gh` commands are refused with guidance to use `-R` or change directory. Outside `~/workspaces`, the shim still falls through to the real guest executable when available. No project is guessed. |
