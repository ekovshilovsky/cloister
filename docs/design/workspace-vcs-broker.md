...
One standalone host service runs per profile and listens on a random loopback port. It follows the same detached host-service pattern as `op-forward` and `cc-clip`: the service has its own process lifetime, its tunnel is detached, and profile lifecycle operations own startup and teardown. Cloister records the service PID, build identity, workspace configuration hash, random token, and exact reverse-tunnel claim under the private state directory. Interactive entry, headless entry, `open`, `exec`, `create`, `repair`, `rebuild`, `reset`, `resize`, `snapshot`, and `add-stack` ensure the service after the VM is available. An ensure failure is a visible warning and does not block VM access. `stop`, `delete`, `rebuild`, `reset`, `resize`, `snapshot`, and an `add-stack` restart synchronously stop it before stopping or destroying the VM. Generic `tunnel.StopAll` deliberately skips the broker-owned claim so a future lifecycle caller cannot bypass the broker drain. The service remains available with no interactive session and therefore supports a headless guest agent.

Ensure-and-deploy is serialized by a per-profile host `flock`. Acquisition is bounded at 12 seconds. The lock covers state inspection, repair or transition requests, and a replacement's bounded startup; it never covers a graceful drain. A caller that finds an intentional drain receives a retryable warning instead of waiting behind it. Starting a new service has a 30-second readiness bound, which covers its independently bounded 12-second guest deployment and subsequent 12-second guest-path probe. The longest failed dead-process recovery can take 79.8 seconds of broker coordination: 12 seconds resolving guest home, 12 seconds waiting for the lock, 1.8 seconds for three host probes, 12 seconds for owner-checked stale guest cleanup, 30 seconds for replacement readiness, and a final 12-second owner-checked guest cleanup if that startup times out after publishing state. Successful recovery omits the final cleanup and is bounded at 67.8 seconds. A healthy inspection whose guest endpoint stalls takes less because it does not perform cleanup or synchronous replacement. Workspace discovery and hypervisor operations outside this broker coordination have their own bounds. A workspace-mode change away from the broker requests daemon-owned retirement and returns without waiting for its drain.
...
The workspace hash covers the guest home, workspace configuration, and derived project session specifications. On a hash mismatch, ensure writes the desired configuration and asks the running daemon to apply it; on a build-identity mismatch it asks for replacement by the current Cloister build. The request returns immediately while the old broker continues serving its current project set. A pure addition is a strict superset of project specifications in which every existing specification is byte-identical and the guest home, workspace configuration, backend, and build identity are unchanged. The daemon applies that case by building and atomically publishing a new mapper and refreshing the guest shim/config with the existing token while admission remains open. Existing handlers retain the proxy snapshot they already acquired.

Removals, changed project roots or modes, and build or unhealthy-daemon replacement use a closing gate. The daemon returns authenticated `503` plus `Retry-After` before any rejected command executes, and drains the set it already admitted. With no new admissions, steady traffic cannot starve the transition. A same-build configuration change rebuilds and swaps the mapper in process and refreshes upgraded guest shims without changing the token. A build change starts the current executable as the replacement and transfers tunnel/config ownership after the old command set drains. Failed or timed-out transitions reopen the old service but remain pending; they retry after exponential backoff beginning at five seconds and capped at one minute. The next ensure and `cloister status` surface the pending failure instead of allowing an obsolete binary or mapper to persist silently. The admission-closed window is bounded at ten minutes for the longest admitted command plus at most 12 seconds for same-build guest refresh; build replacement is bounded at ten minutes plus the 30-second startup bound. Keeping the fixed guest listener continuously open during a process replacement would require descriptor passing or a separate permanent front proxy; that complexity is disproportionate to a rare upgrade and would itself become a second credential-bearing lifecycle component. The build identity is the embedded Cloister version (or Go VCS build metadata for an unversioned development binary), not a hash of the executable.

Shutdown first stops admitting commands while leaving the listener and reverse tunnel available. New authenticated requests receive `503`, `Retry-After`, and a message telling the caller to retry. Already admitted commands run through the host operation and its post-command Mutagen barrier. Command execution is independent of response delivery. Output is written into an unlinked file in a daemon-specific `0700` directory, so `SIGKILL` leaves no named secret-bearing spool; startup also removes any stale entries as a backstop. Storage is capped at 16 MiB per command and 64 MiB for the daemon. At either cap, further output is discarded rather than blocking or killing Git, the command and barrier finish, the real exit code is returned, and the response explicitly states how many bytes were retained. A network write that makes no progress is bounded at 15 seconds, while the deadline slides for a slow reader that continues making progress.
