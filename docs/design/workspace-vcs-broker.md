...
One standalone host service runs per profile and listens on a random loopback port. Cloister records its PID, configuration hash, token identity, and owned SSH reverse tunnel under the private state directory. Entry, exec, open, headless start, and repair serialize an authenticated ensure operation: a healthy service with the current workspace hash is reused without changing its token or guest configuration; a failed end-to-end probe or changed hash causes a controlled restart. The service remains available when no interactive session exists and stops when its profile VM is stopped, deleted, or rebuilt.

SSH exposes the service on a fixed guest loopback port. A random bearer token owned by the service is written into a guest file readable only by the VM user. Health is authenticated through that guest endpoint, covering the guest configuration, tunnel, host listener, and handler rather than trusting a PID or listening port alone. The endpoint accepts only `git` and `gh`, bounded request fields, and a valid token. Git and implicitly targeted repository commands also require a registered workspace path; account-scoped `gh` and explicitly targeted repository commands do not. It never accepts a shell command string. Arguments are transported as repeated URL-encoded values and passed directly to `exec.Cmd`.
