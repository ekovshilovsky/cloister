...
One standalone host service runs per profile and listens on a random loopback port. It follows the same detached host-service pattern as `op-forward` and `cc-clip`: the service has its own process lifetime, its tunnel is detached, and profile lifecycle operations own startup and teardown. Cloister records the service PID, build identity, workspace configuration hash, random token, and exact reverse-tunnel claim under the private state directory. Interactive entry, headless entry, `open`, `exec`, `create`, `repair`, `rebuild`, `reset`, `resize`, `snapshot`, and `add-stack` ensure the service after the VM is available. An ensure failure is a visible warning and does not block VM access. `stop`, `delete`, `rebuild`, `reset`, `resize`, `snapshot`, and an `add-stack` restart synchronously stop it before stopping or destroying the VM. The service remains available with no interactive session and therefore supports a headless guest agent.

Ensure-and-deploy is serialized by a per-profile host `flock`. Acquisition is bounded at 12 seconds. The lock covers state inspection, repair or restart requests, and an unhealthy replacement's bounded startup; it never covers a graceful drain. A caller that finds an intentional drain receives a retryable warning instead of waiting behind it. Starting a new service has a 10-second readiness bound. The bounded lock, three host probes, and startup can add at most 23.8 seconds on the external replacement path, apart from backend SSH connection time used to resolve guest configuration. A workspace-mode change away from the broker requests daemon-owned retirement and returns without waiting for its drain.

Health has two separate failure domains. Daemon health is an authenticated request directly to its host loopback port using the recorded token. Cloister makes three attempts, each bounded at 500 ms, with 100 ms and 200 ms backoffs; all three must fail before the daemon is replaced, for a maximum 1.8-second host-health delay. An authenticated `503` with `Retry-After` identifies an intentional drain and is not treated as a dead daemon. Tunnel health separately verifies the exact recorded SSH process and then sends an authenticated probe through the fixed guest endpoint. A healthy daemon with a failed tunnel path receives an in-process tunnel-repair request; its PID and token stay unchanged. The request remains pending while a host command is active, and the server continues accepting work rather than entering a drain. At the first idle point the daemon briefly closes admission, replaces only its exact tunnel claim, atomically refreshes the guest shims and owner configuration with the existing token, probes the repaired guest path, publishes the new tunnel PID, and resumes admission. No admitted host command or response loses its tunnel to repair.

The workspace hash covers the guest home, workspace configuration, and derived project session specifications. On a hash mismatch, ensure writes the desired configuration and asks the running daemon to apply it; on a build-identity mismatch it asks for replacement by the current Cloister build. The request returns immediately while the old broker continues serving its current project set. The daemon atomically stops new admissions only after it is idle. A same-build configuration change rebuilds and swaps the mapper in process, and also refreshes upgraded guest shims without changing the token. A build change starts the current executable as the replacement and transfers tunnel/config ownership while idle. Failed reload or replacement resumes the old service. Thus a started command is never interrupted for configuration or version replacement; new commands may receive a clear retryable `503` only during the short idle transition. The build identity is the embedded Cloister version (or Go VCS build metadata for an unversioned development binary), not a hash of the executable.

Shutdown first stops admitting commands while leaving the listener and reverse tunnel available. New authenticated requests receive `503`, `Retry-After`, and a message telling the caller to retry. Already admitted handlers run through the host command, its post-command Mutagen barrier, and HTTP response completion. Only then does the daemon atomically remove its owned guest configuration, stop its exact tunnel, and exit. The drain bound is ten minutes, chosen to cover realistic authenticated pushes and hooks without allowing VM lifecycle operations to wait forever; the lifecycle controller allows five additional seconds for cleanup. `cloister stop` and other destructive lifecycle operations wait for that drain. If it expires, Cloister records and reports every interrupted command and guest project path, kills the daemon's process group so a child cannot continue mutating a repository, warns, and continues stopping or destroying the VM. Managed hard-stop paths also target the process group. Each host command also has a sibling watchdog connected to the daemon by a private pipe; if even a raw `SIGKILL` removes only the daemon PID, pipe closure makes the watchdog kill the remaining private process group before the child can continue independently.
